...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem">
  ...
  <div class="panel">
    <div class="panel-head">
      ...
      <a href="index.php?mod=member" class="panel-link">
        Lihat Semua
    ...
    <div class="tbl-wrap">
      <table>
        ...
        <tbody>
          ...
          <?php else: foreach ($stats['recent_members'] as $m):
            ...
            <tr>
              <td>
                <div style="display:flex;align-items:center;gap:10px;">
                  ...
                  <strong style="font-size:.82rem"><?= htmlspecialchars($m['full_name']) ?></strong>
                </div>
              </td>
              <td style="color:var(--muted);font-size:.76rem"><?= htmlspecialchars($m['institution'] ?? '-') ?></td>
  ...
  <div class="panel">
    <div class="panel-head">
      ...
      <a href="index.php?mod=kelas" class="panel-link" style="color:var(--mint)">
        Kelola
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9,18 15,12 9,6"/></svg>
      </a>
    </div>
    <div class="tbl-wrap">
      <table>
        ...
        <tbody>
          <?php if (empty($stats['recent_kelas'])): ?>
            <tr><td colspan="3"><div class="empty">Belum ada kelas</div></td></tr>
          <?php else: foreach ($stats['recent_kelas'] as $k):
            $st = $k['status'] ?? 'draft';
          ?>
            <tr>
              <td><strong style="font-size:.82rem"><?= htmlspecialchars($k['title']) ?></strong></td>
              <td style="color:var(--muted);font-size:.76rem"><?= htmlspecialchars($k['category'] ?? '-') ?></td>
