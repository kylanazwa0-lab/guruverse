<?php
// views/mengajar_gamifikasi.php — Manajemen Gamifikasi (Stats & Tantangan)
require_once __DIR__ . '/../controllers/MengajarGamifikasiController.php';

// Check if tables exist, if not, skip errors or initialize
// Ideally schema.sql is already run, but we suppress errors if it hasn't
try {
    $members_list = gamifikasi_get_members($conn);
} catch (Exception $e) {
    $members_list = [];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create') {
            gamifikasi_create_tantangan($conn, $_POST);
            $msg = 'Tantangan berhasil ditambahkan!';
        } elseif ($action === 'update') {
            gamifikasi_update_tantangan($conn, $_POST['id'], $_POST);
            $msg = 'Tantangan berhasil diperbarui!';
        } elseif ($action === 'delete') {
            gamifikasi_delete_tantangan($conn, $_POST['id']);
            $msg = 'Tantangan berhasil dihapus!';
        } elseif ($action === 'import_game') {
            if (isset($_FILES['gamifikasi_file'])) {
                $uploadRes = gamifikasi_upload_definition($_FILES['gamifikasi_file'], $_POST);
                $msg = $uploadRes['message'];
            }
        } elseif ($action === 'build_game') {
            $buildRes = gamifikasi_generate_json($_POST);
            $msg = $buildRes['message'];
        } elseif ($action === 'delete_game') {
            $delRes = gamifikasi_delete_game($_POST['game_id'] ?? '');
            $msg = $delRes['message'];
        } elseif ($action === 'update_game') {
            $updateRes = gamifikasi_update_game($_POST);
            $msg = $updateRes['message'];
        } elseif ($action === 'delete_game_bulk') {
            if (!empty($_POST['game_ids'])) {
                $delRes = gamifikasi_delete_game_bulk($_POST['game_ids']);
                $msg = $delRes['message'];
            }
        } elseif ($action === 'delete_bulk') {
            if (!empty($_POST['ids'])) {
                gamifikasi_delete_tantangan_bulk($conn, $_POST['ids']);
                $msg = count($_POST['ids']) . ' Tantangan berhasil dihapus!';
            }
        }

    } catch (Exception $e) {
        $msg = 'Terjadi kesalahan: Pastikan tabel gb_mengajar_tantangan sudah dibuat.';
    }
}
...
$stats_list = [];
try {
    $tantangan_list = gamifikasi_get_tantangan($conn, $search);
    $stats_list = gamifikasi_get_stats($conn);
} catch (Exception $e) {
    // If tables don't exist yet, it will fail silently here and show empty
}
?>

<?php
$isError   = !empty($msg) && (strpos(strtolower($msg), 'gagal') !== false || strpos(strtolower($msg), 'tidak valid') !== false);
$isSuccess = !empty($msg) && !$isError;
?>
<?php if ($isSuccess): ?>
<div style="display:flex;align-items:center;gap:12px;background:rgba(6,214,160,0.12);border:1px solid rgba(6,214,160,0.4);border-left:4px solid #06d6a0;border-radius:10px;padding:14px 18px;margin-bottom:1.2rem;">
  ...
  <div><div style="font-weight:700;font-size:.88rem;color:#06d6a0">Upload Berhasil!</div><div style="font-size:.8rem;color:var(--muted);margin-top:2px"><?= htmlspecialchars($msg) ?></div></div>
</div>
<?php elseif ($isError): ?>
<div style="display:flex;align-items:center;gap:12px;background:rgba(239,68,68,0.10);border:1px solid rgba(239,68,68,0.3);border-left:4px solid #ef4444;border-radius:10px;padding:14px 18px;margin-bottom:1.2rem;">
  ...
  <div><div style="font-weight:700;font-size:.88rem;color:#ef4444">Upload Gagal</div><div style="font-size:.8rem;color:var(--muted);margin-top:2px"><?= htmlspecialchars($msg) ?></div></div>
</div>
<?php endif; ?>
...
<div class="panel" style="margin-bottom: 2rem;">
  ...
  <div class="tbl-wrap">
    <table>
      ...
      <tbody>
        <?php
          $katalogPath = __DIR__ . '/../../asset/docs/gamifikasi/gamifikasi_katalog.json';
            ...
            <td style="text-align:center;">
              <div class="aksi-dropdown" style="position:relative;display:inline-block;">
                ...
                <div class="aksi-menu" style="display:none;position:absolute;right:0;top:calc(100% + 5px);min-width:160px;background:#ffffff;border:1px solid var(--border);border-radius:9px;box-shadow:0 8px 28px rgba(0,0,0,.15);z-index:999;overflow:hidden;">
                  <?php if($k['tipe'] === 'json'): ?>
                  <a href="/guruverse/member/pages/Guru_Gamifikasi/index.php?play=<?= urlencode($k['id']) ?>" class="aksi-item" style="display:flex;align-items:center;gap:8px;padding:9px 14px;font-size:.76rem;color:#3b82f6;text-decoration:none;transition:.12s;">
...
<div class="panel">
  ...
  <div class="tbl-wrap">
    <table>
      ...
      <tbody>
        <?php if (empty($tantangan_list)): ?>
          <tr><td colspan="7"><div class="empty">Belum ada tantangan.</div></td></tr>
        <?php else: foreach ($tantangan_list as $i => $k):
