<?php
// views/kelas.php — Manajemen Kelas CRUD
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    if ($action === 'create') {
        kelas_create($conn, $_POST);
        $msg = 'Kelas berhasil ditambahkan!';
    } elseif ($action === 'update') {
        kelas_update($conn, $_POST['id'], $_POST);
        $msg = 'Kelas berhasil diperbarui!';
    } elseif ($action === 'delete') {
        kelas_delete($conn, $_POST['id']);
        $msg = 'Kelas berhasil dihapus!';
    }
}

$search = $_GET['search'] ?? '';
$kelas_list = kelas_get_all($conn, $search);
?>
  ...
  <div class="tbl-wrap">
    <table>
      ...
      <tbody>
        ...
        <?php else: foreach ($kelas_list as $i => $k):
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="modal-body">
        ...
        <div style="font-weight:700; margin-bottom:10px; display:flex; justify-content:space-between;">
          ...
          <a href="/guruverse/uploads/cert_templates/contoh_template.png" target="_blank" style="font-size:0.75rem; color:var(--v1); font-weight:normal; text-decoration:underline;">Lihat Contoh Desain</a>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:.65rem">
...
function openEditKelas(k) {
  ...
  document.getElementById('edit-kelas-cert-template-url').value = k.cert_template ? '/guruverse/uploads/cert_templates/' + k.cert_template : '';
  
  document.getElementById('modal-kelas-edit').style.display = 'flex';
}
</script>

<?php include 'cert_designer.php'; ?>
