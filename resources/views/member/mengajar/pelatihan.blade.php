...
@section('title', 'Pelatihan Offline')
@section('content')
<div class="page active" id="page-pelatihan">
        ...
        <div style="display:flex; flex-direction:column; gap:12px;">
          ...
          <div style="border-radius:12px; border:1px solid var(--c-border-light); background:linear-gradient(135deg, rgba(109,40,217,.02), rgba(139,92,246,.05)); padding:16px;">
            ...
            <div style="display:flex; align-items:center; justify-content:space-between; border-top:1px dashed var(--c-border); padding-top:12px;">
              ...
              <button class="btn btn-sm" style="background:var(--c-primary-pale); color:var(--c-primary); border:none; padding:4px 10px;" onclick="unduhSertifikat('<?= $c['id'] ?>')">Unduh PDF</button>
            </div>
          </div>
          <?php endforeach; ?>
          <?php endif; ?>
...
async function daftarPelatihan(id, title) {
    ...
    try {
        const res = await fetch('/member/mengajar/pelatihan/batches/' + id);
...
async function submitPendaftaran() {
    ...
    try {
        const res = await fetch('/member/mengajar/pelatihan/register', {
...
async function lihatTiket(id) {
    try {
        const res = await fetch('/member/mengajar/pelatihan/ticket/' + id);
        const data = await res.json();
        if(data.status === 'success') {
            const t = data.data;
...
function unduhSertifikat(id) {
    document.getElementById('certModalId').innerText = id;
    document.getElementById('certModalOverlay').style.display = 'flex';
    
    // Auto trigger download after short delay
    setTimeout(() => {
        window.location.href = `/member/mengajar/pelatihan/sertifikat/${id}/unduh`;
        setTimeout(() => {
...
<!-- CERTIFICATE MODAL -->
<div id="certModalOverlay" style="display:none; position:fixed; inset:0; background:rgba(15,23,42,0.6); backdrop-filter:blur(4px); z-index:9999; align-items:center; justify-content:center;" onclick="if(event.target===this) document.getElementById('certModalOverlay').style.display='none'">
  <div style="background:#fff; border-radius:20px; width:90%; max-width:400px; overflow:hidden; box-shadow:0 20px 40px rgba(0,0,0,0.3); text-align:center; padding:32px 24px;">
    <div style="width:64px; height:64px; background:#fef08a; border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 16px;">
      <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="none" stroke="#ca8a04" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="6"></circle><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"></path></svg>
    </div>
    <h3 style="margin:0 0 8px 0; font-size:1.2rem; font-weight:800; color:#0f172a;">Sedang Diproses</h3>
    <p style="margin:0 0 24px 0; font-size:0.9rem; color:#475569; line-height:1.5;">
      Sertifikat <strong id="certModalId"></strong> Anda sedang dalam proses verifikasi dan penerbitan oleh panitia.<br><br>
      <span style="font-size:0.8rem; opacity:0.8;">Dokumen PDF akan segera tersedia untuk diunduh setelah proses administrasi selesai. Terima kasih atas partisipasi dan kesabaran Anda!</span>
    </p>
    <button onclick="document.getElementById('certModalOverlay').style.display='none'" class="btn btn-primary" style="width:100%; padding:12px; border-radius:10px; font-weight:700;">Baik, Mengerti</button>
  </div>
</div>
