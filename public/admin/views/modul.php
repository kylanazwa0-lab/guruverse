<?php
// views/modul.php — Manajemen Modul (Card Grid Layout dalam Panel)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    if ($action === 'create') {
        modul_create($conn, $_POST, $_FILES['file'] ?? null);
        $msg = 'Modul berhasil ditambahkan!';
    } elseif ($action === 'update') {
        modul_update($conn, $_POST['id'], $_POST);
        $msg = 'Modul berhasil diperbarui!';
    } elseif ($action === 'delete') {
        modul_delete($conn, $_POST['id']);
        $msg = 'Modul berhasil dihapus!';
    }
}

$course_filter = $_GET['course_id'] ?? null;
$modul_list    = modul_get_all($conn, $course_filter);

$courses = [];
$res_c = $conn->query("SELECT id, title FROM gb_courses ORDER BY title ASC");
while ($c = $res_c->fetch_assoc()) $courses[] = $c;

// Icon & color per type
function modul_type_meta($type) {
    $map = [
