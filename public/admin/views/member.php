<?php
// views/member.php - Menggunakan API fetch bawaan lama untuk mempertahankan fitur dinamis (search, pagination live)
?>
<div class="panel">
  ...
  <div class="tbl-wrap">
    <table>
      ...
      <tbody id="table-body">
        <?php for($i=0;$i<6;$i++): ?>
        <tr><?php foreach([24,140,120,90,80,90] as $w): ?><td><div class="skel" style="height:11px;width:<?=$w?>px;background:rgba(0,0,0,0.05);border-radius:4px"></div></td><?php endforeach; ?></tr>
        <?php endfor; ?>
      </tbody>
    </table>
  </div>
      ...
      <div class="modal-body">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.2rem">
          <div class="fg">
            ...
            <input type="text" name="fullName" class="fi" placeholder="Contoh: Budi Santoso, S.Pd." required>
          </div>
          <div class="fg">
          ...
          <div class="fg">
            ...
            <p style="font-size:10px;color:var(--muted2);margin-top:4px">Password akan disimpan sebagai <b>plaintext</b> jika belum di-hash.</p>
          </div>
        </div>
      </div>
...
function loadMembers() {
  ...
  fetch('/guruverse/admin/api/get_members.php', {credentials:'same-origin'})
    .then(r => {
      if (r.status === 401) {
        showTableErr('Sesi admin tidak aktif. Silakan <a href="../admin.php" style="color:var(--acc);text-decoration:underline">login kembali</a>.');
        return null;
      }
      ...
      <td style="text-align:center">
        <div class="dropdown relative inline-flex">
          ...
          <ul class="dropdown-menu dropdown-open:opacity-100 hidden min-w-40 z-50" role="menu" aria-orientation="vertical" aria-labelledby="dd-${m.memberId}">
            ...
            <li><a class="dropdown-item text-red-500 hover:bg-red-50" href="#" onclick="deleteMember('${m.memberId}', '${esc(name)}');return false;">Hapus Anggota</a></li>
          </ul>
        </div>
      </td>
    </tr>`;
...
function saveAdd(e) {
  ...
  fetch('/guruverse/admin/api/add_member.php', {
    method: 'POST',
    body: fd
  })
  .then(r => r.json())
...
function doImport(e) {
  ...
  const fd = new FormData(e.target);
  fetch('/guruverse/admin/api/import_member.php', {
    method: 'POST',
    body: fd
  })
  .then(r => r.json())
...
function sendWaInvite(memberId, name, phone) {
  ...
  // Dapatkan base path dinamis sebelum folder 'admin'
  let path = window.location.pathname;
  let adminIndex = path.indexOf('/admin');
  let basePath = adminIndex !== -1 ? path.substring(0, adminIndex) : '';
...
function deleteMember(memberId, name) {
  ...
  if (textEl) textEl.innerText = 'Hapus anggota "' + name + '"? Tindakan ini tidak dapat dibatalkan.';
  ...
  confirmBtn.onclick = () => {
    ...
    fetch('/guruverse/admin/api/delete_member.php', {
      method: 'POST',
    ...
    .then(r => r.json())
    .then(data => {
      if (data.success) { showToast('Anggota berhasil dihapus'); loadMembers(); }
