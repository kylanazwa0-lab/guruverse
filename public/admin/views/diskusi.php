...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'delete') {
        $id = (int)$_POST['id'];
        $conn->query("DELETE FROM gb_discussion_replies WHERE discussion_id=$id");
        $conn->query("DELETE FROM gb_discussions WHERE id=$id");
        echo "<script>window.location='?mod=diskusi';</script>";
        exit;
    }
    elseif ($action === 'reply') {
        $did = (int)$_POST['discussion_id'];
        $body = $conn->real_escape_string($_POST['body']);
        
        $attachment_path = 'NULL';
        if (isset($_FILES['attachment']) && $_FILES['attachment']['error'] == UPLOAD_ERR_OK) {
            $ext = strtolower(pathinfo($_FILES['attachment']['name'], PATHINFO_EXTENSION));
            $filename = 'reply_' . time() . '_' . rand(1000,9999) . '.' . $ext;
            $upload_dir = __DIR__ . '/../../uploads/discussions/';
            if (!is_dir($upload_dir)) mkdir($upload_dir, 0777, true);
            if (move_uploaded_file($_FILES['attachment']['tmp_name'], $upload_dir . $filename)) {
                $attachment_path = "'" . $conn->real_escape_string('/guruverse/uploads/discussions/' . $filename) . "'";
            }
        }
        ...
        trigger_guruverse_bot($did, $body, $conn);
        // --- END BOT ---

        echo "<script>window.location='?mod=diskusi&topic_id=$did';</script>";
        exit;
    }
...
if ($q !== '') {
    ...
    $where .= " AND (d.title LIKE '%$esc_q%' OR m.full_name LIKE '%$esc_q%')";
}

$res = $conn->query("
    SELECT d.*, m.full_name as author_name
...
");
$disks = [];
while ($r = $res->fetch_assoc()) $disks[] = $r;
...
if ($topic_id > 0) {
    ...
    if ($active_topic) {
        $resR = $conn->query("
            SELECT r.*, m.full_name as replier_name 
            FROM gb_discussion_replies r
            LEFT JOIN members m ON r.user_id = m.id
            WHERE r.discussion_id = $topic_id
            ORDER BY r.created_at ASC
        ");
        while($r = $resR->fetch_assoc()) $replies[] = $r;
    }
}
?>
<style>
...
<div class="wa-layout">
  ...
  <div class="wa-main">
    ...
    <?php else: 
      ...
      $opColor = ['#8b2fc9','#06d6a0','#f8961e','#4cc9f0','#ef233c'][crc32($opName) % 5];
    ?>
      <!-- Header Right -->
